...
// Div 동적생성
    ...
    const btnPrev = createAndAppend(navDiv, 'button', 'prev' , 'btn-prev');
    const todayDate = createAndAppend(navDiv, 'div');
    const btnNext = createAndAppend(navDiv, 'button', 'next' , 'btn-next');
    ...
    const render = (year , month)=> {
        todayDate.textContent =`${year}년 ${month}월`;
        ...
        days.forEach((item ,idx) =>{
            html += `<th class="${idx === 0 ? 'sunday ' : idx === days.length - 1 ? 'saturday' : ''}">${item}</th>`;
        });
        html += '</thead>';
